...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    if (updatedAnime) {
      const obtainedAnime = await Anime.findOne({
        malId: updatedAnime.animeId,
      });
      ...
      let flag = false;
      savedAnimeList!.forEach((anime: any) => {
        if (anime.animeId !== updatedList.animeId) {
          anime = updatedList;
          flag = true;
        }
      });
      if (flag == false) savedAnimeList!.push(updatedList);
    }
    user.set({
      animelist: savedAnimeList!,
