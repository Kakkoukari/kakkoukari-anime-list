...
const AnimeListContainer = () => {
  ...
  const [obtainedAnimes, setObtainedAnimes] = useState(undefined);
  const [animeNameTitles, setAnimeNameTitles] = useState(undefined);
  const handleChange = (e) => {
    setSearch(e.target.value);
    console.log(e.target.value);
    if (obtainedAnimes !== undefined && obtainedAnimes.length > 0) {
      setAnimeNameTitles(
